...
for colum in ["Area", "Nivel Educacao", "Localizacao"]:
    df[colum] = df[colum].str.lower().str.strip().str.replace(" ", "_")

with open("profissoes.pl", "w") as f:
    for _,row in df.iterrows():
        f.write(f"profissao({row["Area"]}, {row["Experiencia"]}, {row["Habilidades"]}, {row["Nivel Educacao"]}, {row["Localizacao"]}, {row["Salario"]}).\n")